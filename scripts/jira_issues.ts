...
//  <xbar.var>string(JIRA_JQL_QUERY=""): Jira variable.</xbar.var>
...
const issueToMenu = (issue: Issue) => {
    ...
    if (issue.fields.customfield_10020 && issue.fields.customfield_10020.some((item) => item.state === 'active')) {
        text = `${text} ${issue.fields.customfield_10020.map((item) => item.name).join(', ')} - `;
    }
...
const { issues } = await jiraClient.searchIssues(Deno.env.get('JIRA_JQL_QUERY') || '');
